...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
	{
		...
		for(int i = 0; i < noteCount; i++)
		{
			...
			if(PtInRect(&notes_true[i].rect,ptClick))
			{
...
////////////////////////////////////////////////////
    ...
    //Draw notes to main window
        ...
        RECT saveButton;
        buttonSize = 25;
        saveButton.left = notes_true[i].rect.right - buttonSize *2;
        saveButton.top = notes_true[i].rect.top;
        saveButton.right = notes_true[i].rect.right;
        saveButton.bottom = notes_true[i].rect.top + buttonSize *2;
    ...
    case WM_APP_NOTE_CLOSED:
    {
        HWND noteChild = (HWND)lParam;
			...
			for (int i = 0; i < noteCount;i++) {
			if (notes_true[i].text == noteChild && !(notes_true[i].textLen>0)) {
    ...
    case WM_APP_SAVE:
    {
        ...
        char sql[512];
        //noteCount;
        //notes_true = realloc(notes_true, sizeof(struct Note) * noteCount);
...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow)
{
   ...
   if (sqlite3_prepare_v2(db, sql, -1, &stmt, NULL) != SQLITE_OK) {
       ...
       int i = 0;
       while (sqlite3_step(stmt) == SQLITE_ROW && i < noteCount) {
   ...
   for(int i = 0; i < noteCount; i++){
       ...
       content = getDatabaseEntry(i+1);
       notes_true[i] = (struct Note){rect,"Hello",content,5,i + 1};
       notes_true[i].rect = rect;
       strncpy_s(notes_true[i].title, sizeof(notes_true[i].title), "Hello", _TRUNCATE);
       strncpy_s(notes_true[i].text, sizeof(notes_true[i].text), content, _TRUNCATE);
       notes_true[i].id = i + 1;
